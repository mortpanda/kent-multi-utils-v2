    ...
    switch (this.strUserSession == true) {
      ...
      case true: {
        ...
        if (this.myWebCache == null) {
          ...
          this.dailyWebsites = await this.ProcessArrayService.processWebSites(JSON.parse(strSites), 'Daily Websites');
          this.siteLoaded = true;

        } else {
          const arrCachedSites = await localStorage.getItem('dailySites');
          console.log('Daily sites from storage')
          this.dailyWebsites = await this.ProcessArrayService.processWebSites(JSON.parse(arrCachedSites), 'Daily Websites')
          this.siteLoaded = true;
        }
